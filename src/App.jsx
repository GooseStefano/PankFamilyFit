  ...
  return <div className="scrim" onMouseDown={e => e.target === e.currentTarget && onClose()}>
    <section className="sheet" role="dialog" aria-modal="true" aria-labelledby="add-title">
      ...
      <form onSubmit={submit}>{!selected ? <>
        ...
        <div id="amount-error" className="field-error" role="alert">{amountTouched && amountInvalid ? 'Укажи количество больше нуля.' : ''}</div>
...
function RecipeModal({ foods, initial, initialIngredients, currentUser, onClose, onSave }) {
  ...
  return <div className="scrim" onMouseDown={event => event.target === event.currentTarget && onClose()}><section className="sheet recipe-sheet" role="dialog" aria-modal="true" aria-labelledby="recipe-title">
    ...
    <form onSubmit={submit}>
      <label htmlFor="recipe-name">Название<input id="recipe-name" autoFocus value={name} onChange={event => setName(event.target.value)} placeholder="Например, домашний плов" aria-invalid={!name.trim()} aria-describedby="recipe-name-error" /><span id="recipe-name-error" className="recipe-field-error">{!name.trim() ? 'Введи название блюда.' : ''}</span></label>
      <div className="form-grid"><label htmlFor="recipe-category">Категория<select id="recipe-category" value={category} onChange={event => setCategory(event.target.value)}>{CATEGORIES.slice(1).map(item => <option key={item}>{item}</option>)}</select></label>
        <label htmlFor="recipe-weight">Готовый вес, г<input id="recipe-weight" type="number" inputMode="decimal" min="0.1" step="0.1" value={cookedWeight} onChange={event => setCookedWeight(event.target.value)} placeholder="2400" aria-invalid={!Number.isFinite(cookedWeightNumber) || cookedWeightNumber <= 0} aria-describedby="recipe-weight-error recipe-weight-hint" /><span id="recipe-weight-error" className="recipe-field-error">{!Number.isFinite(cookedWeightNumber) || cookedWeightNumber <= 0 ? 'Укажи вес больше нуля.' : ''}</span></label></div>
      <p id="recipe-weight-hint" className="recipe-weight-hint">Взвесь готовое блюдо без кастрюли/формы. Этот вес нужен для расчёта КБЖУ на 100 г.</p>
      {weightWarning && <div className="recipe-warning" role="status">Готовый вес сильно меньше суммы ингредиентов. Проверь, всё ли верно.</div>}
      ...
      {ingredients.length === 0 ? <div className="recipe-empty"><CookingPot aria-hidden="true" /><strong>Ингредиентов пока нет</strong><span>Добавь продукты из базы, чтобы рассчитать блюдо.</span></div> : <div className="recipe-ingredient-list">{prepared.map((row, index) => {
        ...
        return <article className="recipe-ingredient" key={row.id}><div className="recipe-ingredient-title"><div><strong>{row.food?.name || `Ингредиент ${index + 1}`}</strong><span>Поля можно изменить прямо здесь</span></div><div className="ingredient-actions"><button type="button" aria-label={`Редактировать ингредиент ${index + 1}`} onClick={() => document.getElementById(`ingredient-search-${row.id}`)?.focus()}><Pencil /></button><button type="button" aria-label={`Дублировать ингредиент ${index + 1}`} disabled={!row.food} onClick={() => duplicateIngredient(row)}><Copy /></button><button type="button" aria-label={`Удалить ингредиент ${index + 1}`} onClick={() => removeIngredient(row, index)}><Trash2 /></button></div></div>
          ...
          {!row.food ? <div id={`ingredient-error-${row.id}`} className="ingredient-error" role="alert">Выбери продукт.</div> : row.amount <= 0 ? <div id={`ingredient-error-${row.id}`} className="ingredient-error" role="alert">Количество должно быть больше нуля.</div> : <div className="ingredient-nutrition"><span><strong>{round(row.nutrition.calories)}</strong> ккал</span><span>Б <strong>{round(row.nutrition.protein)}</strong></span><span>Ж <strong>{round(row.nutrition.fat)}</strong></span><span>У <strong>{round(row.nutrition.carbs)}</strong></span></div>}
        </article>
      })}</div>}
      <section className="recipe-preview" aria-live="polite" aria-label="Предпросмотр расчёта блюда"><div className="recipe-preview-head"><div><span className="section-label">РАСЧЁТ</span><h3>Предпросмотр</h3></div><span>{ingredients.length} {ingredientWord(ingredients.length)} · {cookedWeightNumber > 0 ? `${cookedWeightNumber} г` : 'вес не указан'}</span></div>
        ...
        <div className="recipe-portions"><div><strong>Порция 100 г</strong><span>{cookedWeightNumber > 0 ? `${round(portion100.calories)} ккал · Б ${round(portion100.protein)} · Ж ${round(portion100.fat)} · У ${round(portion100.carbs)}` : 'Укажи готовый вес'}</span></div><div><strong>Порция 300 г</strong><span>{cookedWeightNumber > 0 ? `${round(portion300.calories)} ккал · Б ${round(portion300.protein)} · Ж ${round(portion300.fat)} · У ${round(portion300.carbs)}` : 'Укажи готовый вес'}</span></div></div>
      </section>
      <div className="recipe-validation" role="status">{valid ? 'Блюдо готово к сохранению.' : 'Заполни название, готовый вес и все ингредиенты.'}</div>
...
function TodayChecklist({ viewer, profile, data, update, date }) {
  ...
  if (recentEntryDays < 3) auto.push('Добавить ещё пару записей для отчётов')
  const currentGoal = getGoal(data.goals, profile.id, date)
  if (!currentGoal.startDate || currentGoal.startDate <= shiftDate(date, -42)) auto.push('Посмотреть цель КБЖУ')
...
function WeeklyReport({ profile, data, openDay }) {
  ...
  return <>
    ...
    <section className="week-summary" aria-labelledby="week-summary-title">
      <div className="week-summary-head"><div><span className="section-label">ОТЧЁТ</span><h2 id="week-summary-title">Как прошла неделя</h2></div><CalendarRange aria-hidden="true" /></div>
...
function NutritionChart({ days }) {
  ...
  if (populated.length < 2) return <div className="analytics-chart-empty">Пока мало данных для анализа. Добавь записи за несколько дней.</div>
...
function AnalyticsPage({ viewer, profile, setProfile, data, embedded = false }) {
  ...
  const insight = !hasEnough ? 'Пока мало данных для вывода. Добавь записи за несколько дней.' : trend.key === 'down' && report.calorieDifference < 0 ? 'Вес снижается, калории в среднем ниже цели.' : trend.key === 'stable' && closeToGoal ? 'Вес стоит, а средние калории близко к цели.' : trend.key === 'up' && report.calorieDifference > 0 ? 'Вес растёт, средние калории выше цели.' : 'По выбранному периоду нет однозначной связи между весом и питанием.'
  const planLink = !weightGoal ? 'Цель по весу пока не задана.' : report.daysWithEntries < 3 || periodWeights.length < 2 ? 'Пока мало данных для сравнения дефицита и того, как меняется вес.' : goalProgress?.statusKey === 'stable' ? 'Вес стоит: по этому периоду заметной динамики пока нет.' : goalProgress?.statusKey === 'close' ? 'Темп близок к плану по доступным измерениям.' : goalProgress?.assessment || 'Пока мало данных для вывода.'
...
function HistoryPage({ viewer, profile, setProfile, data, setDate, goToday }) {
  ...
  return <><PageHead eyebrow={mode === 'reports' ? 'АНАЛИТИКА' : 'ДНЕВНИК'} title={mode === 'reports' ? 'Отчёты' : 'История'} subtitle={mode === 'reports' ? `КБЖУ и вес ${profile.id === 'vika' ? 'Викуси' : 'Дани'}` : `Дневник ${profile.id === 'vika' ? 'Викуси' : 'Дани'}`} />
...
function ProductsPage({ viewer, data, update, notify }) {
  ...
  const bulkUpdate = action => {
    ...
    const text = action === 'archive' ? 'архивировать' : 'убрать из любимых у'
...
function WeightModal({ initial, profile, existingDates, onClose, onSave }) {
  ...
  return <div className="scrim" onMouseDown={event => event.target === event.currentTarget && onClose()}><section className="sheet weight-sheet" role="dialog" aria-modal="true" aria-labelledby="weight-modal-title">
    <header><div><span className="section-label">ВЕС {profile.id === 'vika' ? 'ВИКУСИ' : 'ДАНИ'}</span><h2 id="weight-modal-title">{initial ? 'Изменить запись' : 'Добавить вес'}</h2></div><button className="icon-button" onClick={onClose} aria-label="Закрыть"><X /></button></header>
    <form onSubmit={submit}>
      ...
      <div id="weight-error" className="field-error" role="alert">{touched && invalid ? 'Введи вес от 20 до 500 кг.' : ''}</div>
...
function WeightChart({ entries }) {
  if (entries.length < 2) return <div className="chart-empty"><Scale aria-hidden="true" /><strong>Недостаточно данных для графика</strong><span>Добавь минимум две записи в выбранном периоде.</span></div>
...
function WeightGoalCard({ progress }) {
  if (!progress) return <section className="weight-goal-card"><span className="section-label">ЦЕЛЬ ПО ВЕСУ</span><h2>Цель пока не задана</h2><p>Её можно настроить в разделе «Настройки».</p></section>
  if (progress.current === null) return <section className="weight-goal-card"><span className="section-label">ЦЕЛЬ ПО ВЕСУ</span><h2>Цель: {round(progress.goal.targetWeight)} кг</h2><p>{progress.assessment}</p></section>
  const weeks = Math.max(1, Math.ceil(progress.weeks))
  return <section className="weight-goal-card" aria-label="Прогресс к цели по весу"><div className="weight-goal-head"><div><span className="section-label">ЦЕЛЬ ПО ВЕСУ</span><h2>{round(progress.goal.targetWeight)} кг</h2></div><span>{progress.pace.label}</span></div><div className="weight-goal-grid"><div><span>Сейчас</span><strong>{round(progress.current)} кг</strong></div><div><span>Осталось</span><strong>{round(progress.remaining)} кг</strong></div></div><div className="goal-progress" role="progressbar" aria-label="Примерный прогресс к цели по весу" aria-valuemin="0" aria-valuemax="100" aria-valuenow={round(progress.percent)}><i style={{ width: `${progress.percent}%` }} /></div><div className="weight-goal-footer"><span>Примерный прогресс: {round(progress.percent)}%</span><span>Ориентир: ~{weeks} нед.</span></div><p>{progress.assessment}</p></section>
}
...
function WeightPage({ viewer, profile, setProfile, data, update, notify }) {
  ...
  return <>
    <PageHead eyebrow="ПРОГРЕСС" title="Вес" subtitle={`Как меняется вес ${profile.id === 'vika' ? 'Викуси' : 'Дани'}`} />
    ...
    </section> : <div className="weight-empty"><Scale aria-hidden="true" /><h2>Записей веса пока нет</h2><p>Добавь первое измерение, чтобы начать следить за динамикой.</p></div>}
    ...
    <section className="weight-list-card" aria-labelledby="weight-list-title">
      ...
      </article>)}</div> : <div className="list-empty">Здесь появятся твои измерения.</div>}
    </section>
...
function WeightGoalSettings({ viewer, data, update, notify }) {
  ...
  const save = event => {
    ...
    notify('Цель по весу сохранена')
  }
  return <section className="settings-card weight-goal-settings"><div className="card-title"><div><span className="section-label">ЦЕЛЬ ПО ВЕСУ</span><h2>План снижения веса</h2></div><Scale aria-hidden="true" /></div>{viewer.role === 'admin' && <div className="profile-switch compact">{USERS.map(user => <button key={user.id} className={profileId === user.id ? 'active' : ''} onClick={() => select(user.id)}>{user.name}</button>)}</div>}<p className="form-hint">Текущий вес: {current === null ? 'нет записи — сначала добавь измерение' : `${round(current)} кг · ${prettyDate(stats.latest.date)}`}</p><form onSubmit={save}><label>Целевой вес, кг<input type="number" inputMode="decimal" min="20" max="500" step="0.1" value={form.targetWeight} onChange={event => setForm({ ...form, targetWeight: event.target.value })} placeholder="Например, 75" aria-invalid={Boolean(form.targetWeight) && invalid} aria-describedby="weight-goal-error" /></label><div id="weight-goal-error" className="field-error" role="alert">{!current ? 'Добавь текущий вес, затем укажи цель.' : form.targetWeight && invalid ? 'Для плана снижения цель должна быть меньше текущего веса и не ниже 20 кг.' : ''}</div><label>Желаемый темп<select value={form.pace} onChange={event => setForm({ ...form, pace: event.target.value })}>{Object.entries(WEIGHT_PACES).map(([key, pace]) => <option key={key} value={key}>{pace.label}</option>)}</select></label><label>Дата начала<input type="date" max={todayISO()} value={form.startDate} onChange={event => setForm({ ...form, startDate: event.target.value })} /></label><button className="primary wide" disabled={invalid}>Сохранить цель по весу</button></form><p className="form-hint">Это ориентир для наблюдения за динамикой, а не медицинское назначение.</p></section>
}
...
function HabitModal({ initial, onClose, onSave }) {
  ...
  return <div className="scrim" onMouseDown={event => event.target === event.currentTarget && onClose()}><section className="sheet habit-sheet" role="dialog" aria-modal="true" aria-labelledby="habit-modal-title"><header><div><span className="section-label">ПРИВЫЧКА</span><h2 id="habit-modal-title">{initial ? 'Изменить привычку' : 'Новая привычка'}</h2></div><button className="icon-button" aria-label="Закрыть" onClick={onClose}><X /></button></header><form onSubmit={submit}><label>Название<input autoFocus value={name} onChange={event => setName(event.target.value)} placeholder="Например, прогулка 20 минут" /></label><label>Кто видит<select value={visibility} onChange={event => setVisibility(event.target.value)}><option value="danya">Даня</option><option value="vika">Викуся</option><option value="both">Оба</option></select></label><fieldset className="habit-days"><legend>Дни недели</legend><div>{WEEKDAYS.map(([key, label]) => <button type="button" key={key} className={weekdays.includes(key) ? 'active' : ''} aria-pressed={weekdays.includes(key)} onClick={() => toggleDay(key)}>{label}</button>)}</div></fieldset><div className="field-error" role="alert">{!clean || !weekdays.length ? 'Укажи название и хотя бы один день.' : ''}</div><button className="primary wide" disabled={!clean || !weekdays.length}>{initial ? 'Сохранить привычку' : 'Создать привычку'}</button></form></section></div>
}
...
function AppInfo({ mode, isOffline, lastSuccessfulSync, updateAvailable, onUpdate }) {
  ...
  return <section className="settings-card app-info" aria-labelledby="app-info-title"><div className="card-title"><div><span className="section-label">ПРИЛОЖЕНИЕ</span><h2 id="app-info-title">Версия и обновления</h2></div><RefreshCw aria-hidden="true" /></div><dl><div><dt>Версия</dt><dd>{APP_VERSION}</dd></div><div><dt>Данные</dt><dd>{mode === 'supabase' ? 'Supabase' : 'Локально'}</dd></div><div><dt>Сеть</dt><dd>{isOffline ? 'Оффлайн' : 'Онлайн'}</dd></div><div><dt>Последняя синхронизация</dt><dd>{mode === 'supabase' ? syncTime : 'Не требуется'}</dd></div></dl>{updateAvailable && <div className="app-update" role="status" aria-atomic="true"><div><strong>Доступно обновление приложения</strong><span>Новая версия уже готова.</span></div><button className="primary" onClick={onUpdate}><RefreshCw aria-hidden="true" />Обновить</button></div>}<p className="form-hint">Если обновление не появилось, открой приложение при доступном интернете и подожди несколько секунд.</p></section>
}
...
function SettingsPage({ viewer, data, update, onLogout, notify, hasLocalMigration, migrateLocalData, dismissLocalMigration, mode, isOffline, lastSuccessfulSync, updateAvailable, onUpdate }) {
  return <><PageHead eyebrow="ПРОФИЛЬ" title="Настройки" subtitle={viewer.id === 'vika' ? 'Ты вошла как Викуся' : 'Ты вошёл как Даня'} />{viewer.role === 'admin' && <NutritionPlanSettings viewer={viewer} data={data} update={update} notify={notify} />}
...
export default function App() {
  ...
  if (loading) return <main className="loading-shell" role="status" aria-live="polite"><div className="brand-mark"><Apple aria-hidden="true" /></div><strong>Открываем наш дневник…</strong><span>Проверяем синхронизацию данных.</span></main>
  return <div className={`desktop-bg ${profile.id === 'vika' ? 'theme-vika' : ''}`}><main className="app-shell"><div className="scroll-area">
    <div className={`storage-status ${mode} ${syncState}`} role="status">{status}</div>
    {isOffline && <section className="offline-notice" role="status" aria-live="polite"><strong>{hasOfflineSnapshot ? 'Показан последний локальный снимок' : 'Нет сохранённого снимка'}</strong><span>{hasOfflineSnapshot ? 'Интернет недоступен. Просмотр работает, а изменения временно заблокированы — синхронизация не обещается.' : 'Открой приложение онлайн хотя бы один раз, чтобы сохранить данные для офлайн-просмотра.'}</span><button className="secondary" onClick={reload}>Проверить ещё раз</button></section>}
    ...
    <fieldset className="offline-readonly" disabled={isOffline} aria-label={isOffline ? 'Пока можно только посмотреть' : undefined}>
