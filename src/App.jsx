...
function WeightPage({ viewer, profile, setProfile, data, update, notify }) {
  ...
  return <>
    <PageHead eyebrow="ПРОГРЕСС" title="Вес" subtitle={`Как меняется вес · ${profile.name}`} />
...
function HabitModal({ initial, onClose, onSave }) {
  ...
  return <div className="scrim" onMouseDown={event => event.target === event.currentTarget && onClose()}><section className="sheet habit-sheet" role="dialog" aria-modal="true" aria-labelledby="habit-modal-title"><header><div><span className="section-label">ПРИВЫЧКА</span><h2 id="habit-modal-title">{initial ? 'Изменить привычку' : 'Новая привычка'}</h2></div><button className="icon-button" aria-label="Закрыть" onClick={onClose}><X /></button></header><form onSubmit={submit}><label>Название<input autoFocus value={name} onChange={event => setName(event.target.value)} placeholder="Например, прогулка 20 минут" /></label><label>Кто видит<select value={visibility} onChange={event => setVisibility(event.target.value)}><option value="danya">Даня</option><option value="vika">Викуся</option><option value="both">Оба</option></select></label><fieldset className="habit-days"><legend>Дни недели</legend><div>{WEEKDAYS.map(([key, label]) => <button type="button" key={key} className={weekdays.includes(key) ? 'active' : ''} aria-pressed={weekdays.includes(key)} onClick={() => toggleDay(key)}>{label}</button>)}</div></fieldset><div className="field-error" role="alert">{!clean || !weekdays.length ? 'Укажите название и хотя бы один день.' : ''}</div><button className="primary wide" disabled={!clean || !weekdays.length}>{initial ? 'Сохранить привычку' : 'Создать привычку'}</button></form></section></div>
}
...
export default function App() {
  ...
  if (loading) return <main className="loading-shell" role="status" aria-live="polite"><div className="brand-mark"><Apple aria-hidden="true" /></div><strong>Открываем ваш дневник…</strong><span>Проверяем синхронизацию данных.</span></main>
