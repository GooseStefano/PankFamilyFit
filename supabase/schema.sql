...
create table food_items (id uuid primary key default gen_random_uuid(), name text not null, type text not null check(type in ('product','dish')), category text not null, base_unit text not null, base_amount numeric not null check(base_amount>0), calories numeric not null, protein numeric not null, fat numeric not null, carbs numeric not null, is_favorite boolean not null default false, created_by uuid references app_users, is_archived boolean not null default false, usage_count integer not null default 0, created_at timestamptz not null default now(), updated_at timestamptz not null default now());
